...
class ImageBatcher:
    ...
    def preprocess_image(self, image_path):
        ...
        * V1: The algorithm for EfficientNet V1, aka "Legacy", as defined in automl/efficientnetv2/preprocess_legacy.py.
        ...
        def normalize_HWC(image:np.ndarray, means:list, stds:list):
            # image: RGB
            image[:,:,2] = (image[:,:,2] - means[2]) / stds[2]  # B
            image[:,:,1] = (image[:,:,1] - means[1]) / stds[1]  # G
            image[:,:,0] = (image[:,:,0] - means[0]) / stds[0]  # R
            return image
        
        def normalize_CHW(image:np.ndarray, means:list, stds:list):
            # image: RGB
            image = (image - means) / stds
            return image
        
        def greyscale_HWC(image:np.ndarray, rgb_weights = [0.2989, 0.5870, 0.1140]):
            # rgb_weights come from following urls:
            # https://en.wikipedia.org/wiki/Luma_%28video%29
            # https://github.com/tensorflow/tensorflow/blob/v2.16.1/tensorflow/python/ops/image_ops_impl.py#L2563-L2596
            # image: RGB
            image[:,:,2] # B
            image[:,:,1]  # G
            image[:,:,0]  # R
            gray_image = image[:,:,2] * rgb_weights[2] + image[:,:,1] * rgb_weights[1] + image[:,:,0] * rgb_weights[0]
            return np.expand_dims(gray_image, axis=-1)

        image = Image.open(image_path)
        image = image.convert(mode="RGB")  # RGB
        if self.preprocessor == "V2":
            ...
            # just rescale image [0.0, 1.0], no more else
            image = image.resize((self.width, self.height), resample=Image.BILINEAR)
            image = np.asarray(image, dtype=self.dtype)
            image = image / 255.0
        elif self.preprocessor == "V4":
            # just rescale image to [-1.0, 1.0], no more else
            image = image.resize((self.width, self.height), resample=Image.BILINEAR)
            image = np.asarray(image, dtype=self.dtype)
        ...
        elif self.preprocessor == 'V5':
            image = image.convert(mode="L").convert(mode="RGB")  # RGB -> Gray -> RGB
            image = image.resize((self.width, self.height), resample=Image.BILINEAR)  # RGB
            image = np.asarray(image, dtype=self.dtype)  # RGB, HWC
            image = image / 255.0  # RGB
            means = [0.485, 0.456, 0.406]  # RGB
            stds = [0.229, 0.224, 0.225]  # RGB
            image = normalize_HWC(image, means, stds)  # normalize image, RGB
        elif self.preprocessor == "V6":
            # just rescale image [0.0, 1.0], no more else
            image = image.resize((self.width, self.height), resample=Image.BILINEAR)
            image = np.asarray(image, dtype=self.dtype)
            image = image / 255.0
            image = greyscale_HWC(image)
            # cv2.imwrite('/data_ws/Data_1/tinghao/temp_files/1.jpg', image)
        else:
