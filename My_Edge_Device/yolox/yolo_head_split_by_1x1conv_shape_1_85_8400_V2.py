...
class YOLOXHead(nn.Module):
    ...
    def decode_outputs(self, outputs, dtype):
        ...
        for (hsize, wsize), stride in zip(self.hw, self.strides):
            yv, xv = meshgrid([torch.arange(hsize), torch.arange(wsize)])
            grid = torch.stack((xv, yv), 2).view(1, 2, -1)
            grids.append(grid)
            shape = (grid.shape[0], grid.shape[-1])
            strides.append(torch.full((1, *shape), stride))
        
        grids = torch.cat(grids, dim=-1).unsqueeze(dim=-1).type(dtype)
        strides = torch.cat(strides, dim=-1).unsqueeze(dim=-1).type(dtype)
        ...
        outputs = torch.cat([
            ...
            torch.exp(wh) * strides,
            conf
        ], dim=1)  # [batch, 85, n_anchors_all, 1]
        return outputs
